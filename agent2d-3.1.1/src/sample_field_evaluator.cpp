...
evaluate_state( const PredictState & state )
{
    ...
    //
    double point =30.0;
    if(state.ball().pos().x < 30)
    point = state.ball().pos().x;
    Vector2D theirTeamGoalFrontpos(40.0,0);
    point += std::max( 0.0,
                       40.0 - theirTeamGoalFrontpos.dist( state.ball().pos() ) );
    ...
    if( (state.offsideLineX() - state.ball().pos().x ) < 1.0 && state.ball().pos().x <35.0){
    	point +=10.0;
    }
